...
export const templateToSvg = (template: SlabTemplate): string => {
  ...
  const warning = (template.warnings ?? [])
    ...
    .join('');
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}mm" height="${height}mm" viewBox="0 0 ${width} ${height}" role="img" aria-label="Printable slab template"><style>.cut{fill:none;stroke:#111;stroke-width:.35}.fold{fill:none;stroke:#555;stroke-width:.25;stroke-dasharray:3 2}.label{font:4px sans-serif;text-anchor:middle;fill:#111}.dim,.warning{font:3.5px sans-serif;fill:#333}.warning{fill:#8a3b12}</style>${paths}${warning}<rect x="${margin}" y="${height - 62}" width="50" height="50" class="cut"/><text x="${margin + 25}" y="${height - 36}" class="label">50 mm calibration</text><text x="${margin}" y="${height - 6}" class="dim">${template.dimensions.width.toFixed(1)} × ${template.dimensions.height.toFixed(1)} mm · Print at 100%</text></svg>`;
};
